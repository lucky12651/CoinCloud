    ...
    >
      <div className="bn-panel">
        <div className="bn-row">
          <div className="bn-label">Hide balances</div>
          <button type="button" className={`bn-coin${hideBalances ? ' on' : ''}`} onClick={toggleHideBalances}>
            {hideBalances ? <EyeOff size={14} /> : <Eye size={14} />}
            {hideBalances ? 'Hidden' : 'Visible'}
          </button>
        </div>
        <div className="bn-row">
          <div className="bn-label">Auto-lock</div>
          <select
            className="bn-input"
